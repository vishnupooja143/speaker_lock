...
- Weighted Overlap Rescue (Context + Volume + Pitch)
"""
...
class SpeakerLock:
    ...
    def process_chunk(self, pcm_bytes: bytes, chunk_ms: float) -> tuple[bool, bool]:
        ...
        embedding, chunk_sfm, chunk_f0 = _compute_embedding(buffered_bytes, self._sample_rate)
        ...
        # ==========================================
        # 🆘 ENHANCED OVERLAP RESCUE PROTOCOL
        # ==========================================
        if not accepted:
            is_borderline = raw_similarity > _HARD_REJECT_COSINE
            
            if is_borderline:
                # Context Check: Was the customer JUST speaking?
                recent_accepts = sum(1 for d in list(self._recent_sims)[-3:] if d > self._cosine_threshold)
                context_bonus = 0.4 if recent_accepts >= 2 else 0.0
                
                # Volume Check
                is_loud = rms_val > (1.5 * self._bg_rms_ema)
                volume_bonus = 0.3 if is_loud else 0.0
                
                # Pitch Check (Wider tolerance for overlap)
                pitch_match = False
                if chunk_f0 > 0 and self._profile_f0_median is not None:
                    tol = max(50.0, self._profile_f0_median * 0.30) # 30% or 50Hz
                    pitch_match = abs(chunk_f0 - self._profile_f0_median) < tol
                pitch_bonus_rescue = 0.3 if pitch_match else 0.0
                
                rescue_score = context_bonus + volume_bonus + pitch_bonus_rescue
