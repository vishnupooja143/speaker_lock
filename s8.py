...
class SpeakerLock:
    ...
    def _process_chunk_during_bot_speech(self, pcm_bytes: bytes, chunk_ms: float) -> tuple[bool, bool]:
        ...
        if self._state != LockState.LOCKED:
            _, chunk_sfm, _, _, _ = _compute_embedding(pcm_bytes, self._sample_rate)
            loud_enough = (
        ...
        # 🆘 OVERLAP RESCUE DURING BOT SPEECH
        if not accepted and raw_similarity > _HARD_REJECT_COSINE:
            ...
            if rms_val > (2.0 * self._bg_rms_ema):
                rescue_score += 0.20
                
            if rescue_score >= 0.60:
                accepted = True
                self._barge_frames += 1
                self._barge_sustain_ms = 0.0
                logger.info(f"🆘 BOT-SPEECH OVERLAP RESCUE: score={rescue_score:.2f}")
