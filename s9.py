"""
speaker_lock.py - ULTRA-LOW LATENCY & ADVANCED OVERLAP RESCUE
=============================================================
Optimizations:
- Sample-and-Hold Architecture (Skips heavy AI on intermediate frames)
- Reduced buffer (100ms) for ultra-fast response
- Weighted Overlap Rescue (Context + Volume + Pitch + Spectral Shape)
- STRICT BOT-SPEECH PROTECTION (Prevents other people/noise from interrupting)
"""
...
_EMBEDDING_BUFFER_MS = int(os.getenv("EMBEDDING_BUFFER_MS", "100")) # Reduced to 100ms
...
# INCREASED FROM 0.40 TO 0.65 TO PREVENT RANDOM NOISE/OTHER PEOPLE FROM INTERRUPTING BOT
...
if _SEPARATION_ENABLED:
    ...
    logger.info("🔇 Separation DISABLED (optimized for speed)")
...
# ======================================================================
# SPEAKER LOCK - CORE CLASS (ULTRA-LOW LATENCY OPTIMIZED)
# ======================================================================
class SpeakerLock:
    def __init__(
        self,
    ...
    def process_chunk(self, pcm_bytes: bytes, chunk_ms: float) -> tuple[bool, bool]:
        ...
        # ===== 6. BUFFERED EMBEDDING & SAMPLE-AND-HOLD (ULTRA FAST) =====
        ...
        if self._buffer_chunk_count < _EMBEDDING_BUFFER_CHUNKS:
            # Buffer NOT full yet. Skip heavy AI computation.
            # Just repeat the last decision (Sample and Hold).
            if self._last_decision is None:
                # First chunk, default to accept
                self._accepted += 1
        ...
        # Compute embedding (Happens only every 100ms)
        ...
        # Fast path (skip checks if highly confident and recent)
        ...
        # ==========================================
        # 🆘 ADVANCED OVERLAP RESCUE PROTOCOL (GPU-READY & ROBUST)
        # ==========================================
        if not accepted:
            ...
            if is_borderline:
                ...
                # 1. Context Check: Was the customer JUST speaking?
                recent_accepts = sum(1 for d in list(self._recent_sims)[-3:] if d > self._cosine_threshold)
                if recent_accepts >= 2:
                    rescue_score += 0.35
                
                # 2. Volume & Energy Check: Is the user dominating the mix?
                is_loud = rms_val > (1.8 * self._bg_rms_ema)
                if is_loud:
                    rescue_score += 0.25
                
                # 3. Pitch (F0) Dominance Check (Highly robust during overlap)
                if chunk_f0 > 0 and self._profile_f0_median is not None:
                    tol = max(40.0, self._profile_f0_median * 0.25) # 25% or 40Hz tolerance
                    if abs(chunk_f0 - self._profile_f0_median) < tol:
                        rescue_score += 0.30
                    elif abs(chunk_f0 - self._profile_f0_median) < tol * 1.5:
                        rescue_score += 0.15

                # 4. Spectral Shape Check (ZCR & Centroid)
                # These are less affected by linear mixing than full MFCC embeddings
                ...
                # 5. GPU Separation Boost (if separation is active, the incoming audio is already cleaner)
                if _SEPARATION_ENABLED:
                    rescue_score += 0.05

                if rescue_score >= 0.65:
                    accepted = True
                    logger.info(f"🆘 OVERLAP RESCUE TRIGGERED: score={rescue_score:.2f} (sim={raw_similarity:.2f}, f0={chunk_f0:.1f})")

        # Save decision for Sample-and-Hold
    ...
    def _process_chunk_during_bot_speech(self, pcm_bytes: bytes, chunk_ms: float) -> tuple[bool, bool]:
        ...
        # 2. STRICT REQUIREMENT: Must be LOCKED to allow barge-in.
        # This prevents random noise or other people from interrupting the bot during enrollment or if the profile is lost.
        if self._state != LockState.LOCKED:
            self._echo_skipped += 1
            self._note_echo(rms_val)
            self._barge_sustain_ms = 0.0
        ...
        # 3. We are LOCKED. Compute full features for strict verification.
        embedding, chunk_sfm, chunk_f0, chunk_zcr, chunk_centroid = _compute_embedding(pcm_bytes, self._sample_rate)
        raw_similarity = _cosine_similarity(embedding, self._profile)

        # 4. Hard rejects for non-voice or very low similarity (likely another person or noise)
        if chunk_sfm < _SFM_MUSIC_CEILING or chunk_sfm > _SFM_NOISE_FLOOR:
            self._rejected += 1
            self._echo_skipped += 1
            self._note_echo(rms_val)
        ...
        # Base threshold is slightly higher during bot speech
        threshold = self._cosine_threshold + _BOT_SPEAKING_EXTRA_MARGIN
        accepted = raw_similarity >= threshold
        
        # 🆘 STRICT OVERLAP RESCUE DURING BOT SPEECH
        # Only rescue if it's VERY likely the real customer, to prevent other people/noise from interrupting.
        if not accepted and raw_similarity > _HARD_REJECT_COSINE:
            rescue_score = 0.0
            
            # F0 must be a VERY close match (tighter tolerance during bot speech)
            if chunk_f0 > 0 and self._profile_f0_median is not None:
                tol = max(30.0, self._profile_f0_median * 0.15) # 15% or 30Hz tolerance (stricter than normal 25%)
                if abs(chunk_f0 - self._profile_f0_median) < tol:
                    rescue_score += 0.45
                elif abs(chunk_f0 - self._profile_f0_median) < tol * 1.5:
                    rescue_score += 0.20
            
            # ZCR and Centroid must BOTH match closely
            ...
            # Volume must be significantly higher than bot echo to be considered a real barge-in
            if rms_val > (2.5 * self._bg_rms_ema):
                rescue_score += 0.20
                
            # Require a much higher rescue score AND baseline similarity to interrupt the bot
            if rescue_score >= 0.75 and raw_similarity >= (self._cosine_threshold - 0.05):
                accepted = True
                self._barge_frames += 1
                self._barge_sustain_ms = 0.0
                logger.info(f"🆘 STRICT BOT-SPEECH OVERLAP RESCUE: score={rescue_score:.2f} (sim={raw_similarity:.2f})")
    ...
    def _build_profile(self):
        ...
        # Store robust feature medians for overlap rescue
        if zcrs:
            self._profile_zcr_median = float(np.median(zcrs))
            self._profile_zcr_mad = max(float(np.median(np.abs(zcrs - self._profile_zcr_median))) * 1.4826, 0.005)
        else:
            self._profile_zcr_median = 0.0
            self._profile_zcr_mad = 0.01
            
        if centroids:
            self._profile_centroid_median = float(np.median(centroids))
            self._profile_centroid_mad = max(float(np.median(np.abs(centroids - self._profile_centroid_median))) * 1.4826, 0.005)
        else:
            self._profile_centroid_median = 0.0
            self._profile_centroid_mad = 0.01
