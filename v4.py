"""
speaker_lock.py - MAXIMUM ACCURACY Speaker Lock
=================================================
Enhanced with:
- Buffered embeddings (500ms windows instead of 20ms)
- Score smoothing (EMA over decisions)
- SNR-adaptive threshold
- Quality-scored enrollment
- Contamination detection
- Multi-feature scoring (embedding + pitch + energy)
...
# Buffered embedding: accumulate chunks before computing embedding
_EMBEDDING_BUFFER_MS = int(os.getenv("EMBEDDING_BUFFER_MS", "500"))  # 500ms window
_EMBEDDING_BUFFER_CHUNKS = max(1, _EMBEDDING_BUFFER_MS // 20)  # 25 chunks at 20ms

# Score smoothing: EMA over similarity scores
_SIMILARITY_SMOOTH_ALPHA = float(os.getenv("SIMILARITY_SMOOTH_ALPHA", "0.3"))

# SNR-adaptive threshold
_SNR_ADAPTIVE_ENABLED = os.getenv("SNR_ADAPTIVE", "true").lower() == "true"
_SNR_GOOD_DB = 15.0    # Above this = clean audio
_SNR_BAD_DB = 5.0      # Below this = noisy audio
_SNR_THRESHOLD_RELAX = 0.08  # Relax threshold by this much in noise

# Minimum embedding quality for enrollment
...
if _SEPARATION_ENABLED:
    try:
        ...
        logger.warning(f"🔊 Separation ENABLED: {_SEPARATION_MODEL}")
...
logger.info(
    ...
    f"BUFFER={_EMBEDDING_BUFFER_MS}ms, "
    f"SNR_ADAPTIVE={_SNR_ADAPTIVE_ENABLED}, "
    f"SMOOTH_ALPHA={_SIMILARITY_SMOOTH_ALPHA}"
)
...
def set_embedding_backend(fn: Optional[Callable[[np.ndarray], np.ndarray]]) -> None:
    """Replace the built-in embedding extractor with a custom function."""
...
# ======================================================================
# SPEAKER LOCK - CORE CLASS (ENHANCED)
# ======================================================================
class SpeakerLock:
    def __init__(
        self,
    ...
    def _get_adaptive_threshold(self) -> float:
        """
        NEW: SNR-adaptive threshold.
        In noisy conditions, relax the threshold to avoid false rejections.
        In clean conditions, use strict threshold.
        """
        ...
        if _SNR_ADAPTIVE_ENABLED and self._state == LockState.LOCKED:
            ...
            if snr < _SNR_BAD_DB:
                # Very noisy: relax threshold significantly
                threshold -= _SNR_THRESHOLD_RELAX
                logger.debug(f"🔊 Noisy (SNR={snr:.1f}dB), threshold relaxed to {threshold:.2f}")
            elif snr < _SNR_GOOD_DB:
                # Moderately noisy: partial relaxation
    ...
    def _update_similarity_ema(self, raw_similarity: float) -> float:
        """
        NEW: Smooth similarity scores using EMA.
        Prevents frame-by-frame jitter and gives more stable decisions.
        """
    ...
    def _detect_overlap(self) -> bool:
        """
        NEW: Detect if multiple speakers are overlapping.
        Uses variance of recent similarity scores.
        """
        ...
        return variance > 0.02  # High variance = overlap
    ...
    def process_chunk(self, pcm_bytes: bytes, chunk_ms: float) -> tuple[bool, bool]:
        ...
        if self._buffer_chunk_count >= _EMBEDDING_BUFFER_CHUNKS:
            ...
            embedding, chunk_sfm, chunk_f0 = _compute_embedding(
                buffered_bytes, self._sample_rate
            )
            ...
            # Use single chunk embedding until buffer is full
            embedding, chunk_sfm, chunk_f0 = _compute_embedding(
                pcm_bytes, self._sample_rate
            )

        raw_similarity = _cosine_similarity(embedding, self._profile)

        # NEW: Track recent similarities for overlap detection
        self._recent_sims.append(raw_similarity)

        # NEW: Smooth the similarity score
        ...
        # Combine embedding similarity with pitch matching
        pitch_bonus = 0.0
        if chunk_f0 > 0 and self._profile_f0_median is not None:
            ...
            if pitch_diff < pitch_spread:
                pitch_bonus = 0.05  # Pitch matches → boost score
            elif pitch_diff > pitch_spread * 3:
                pitch_bonus = -0.05  # Pitch very different → penalize

        # NEW: Overlap detection and handling
        is_overlap = self._detect_overlap()
        overlap_bonus = 0.0
        if is_overlap:
            # During overlap, give slight benefit to louder voice
            if rms_val > 1.5 * self._bg_rms_ema:
                overlap_bonus = 0.03

        # Final decision using smoothed similarity + bonuses
    ...
    def _observe_similarity(self, raw_similarity: float) -> None:
        ...
        self._adapted = True
        logger.info(f"🎚️ Threshold: {old:.2f} -> {proposed:.2f}")
    ...
    def _build_profile(self):
        """ENHANCED: Quality-scored enrollment with contamination detection."""
        self._contamination_detected = False
        ...
        if len(vecs) >= 3:
            # NEW: Contamination detection via clustering
            ...
            if sims_matrix:
                ...
                # If inter-window similarity is low, likely contamination
        ...
        if self._enroll_f0_samples:
            ...
            if mad > _F0_MAD_SUSPECT_HZ:
                ...
                logger.warning(f"⚠️ LOW-CONFIDENCE profile (f0_mad={mad:.1f}Hz)")
        else:
            self._profile_f0_median = None
...
class SpeakerGatedVAD:
    ...
    def __getattr__(self, name: str) -> Any:
        return getattr(object.__getattribute(self, "_real_vad"), name)
    ...
    def __setattr__(self, name: str, value: Any) -> None:
        if name in ("_real_vad", "_frame_filter", "_bot_speaking_ref",
            ...
            setattr(object.__getattribute(self, "_real_vad"), name, value)

    def stream(self, *args, **kwargs):
        real_vad = object.__getattribute(self, "_real_vad")
        frame_filter = object.__getattribute(self, "_frame_filter")
        bot_speaking_ref = object.__getattribute(self, "_bot_speaking_ref")
        real_stream = real_vad.stream(*args, **kwargs)
        return SpeakerGatedVADStream(real_stream, frame_filter, self, bot_speaking_ref)

    def stats_str(self) -> str:
        sup = object.__getattribute(self, "_suppressed_count")
        pas = object.__getattribute(self, "_passed_count")
        ...
        async for event in self._real_stream:
            if self._is_start_speech(event):
                ...
                if self._should_suppress():
                    ...
                    sup = object.__getattribute(self._gated_vad, "_suppressed_count")
                    object.__setattr__(self._gated_vad, "_suppressed_count", sup + 1)
                    continue
                self._suppressing_current_utterance = False
                pas = object.__getattribute(self._gated_vad, "_passed_count")
