...
# Note: For 3+ speakers, use a 3-source model like "speechbrain/sepformer-wsj03mix"
...
class SpeakerLock:
    ...
    def process_chunk(self, pcm_bytes: bytes, chunk_ms: float) -> tuple[bool, bool]:
        ...
        if _SEPARATION_ENABLED and len(self._utt_sims) >= 8:
            ...
            # High variance indicates multiple overlapping speakers (cocktail party effect)
            if variance > 0.025:  
                # Scale relaxation dynamically (up to 0.15) based on variance severity
        ...
        while True:
            ...
            try:
                ...
                with torch.no_grad():
                    ...
                    # Run separation. 
                    # Note: Ensure SEPARATION_MODEL is a 3-speaker model like 'speechbrain/sepformer-wsj03mix'
                    est_sources = _SEP_MODEL(wav_torch)
                    sources = est_sources.cpu().numpy()[0]  # Shape: (n_sources, samples)
                    n_sources = sources.shape[0]
                    
                    scores = []
                    separated_streams = []
                    
                    # 1. TOURNAMENT: Score ALL separated streams independently
                    ...
                    # 2. ARTIFACT CHECK: Score the original mixed chunk
                    # DNNs sometimes hallucinate or distort audio when separating 3+ voices.
                    orig_score = self._score_stream_full(chunk)
                    
                    # 3. SMART FALLBACK:
                    # If the customer is dominating the mic (loud) and the AI separation 
                    # introduced artifacts (lowering the score), use the raw mix instead.
                    if best_score < _SEP_MIN_FALLBACK_SCORE and orig_score >= best_score:
                        ...
                        # Otherwise, use the isolated stream that matched the customer best
